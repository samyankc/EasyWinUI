...
{
    enum class Error { NotFound, EmptyInput, EmptyTarget, BeforeBegin, ReachingEnd };
    using StrView = std::string_view;
    using Expected = std::expected<StrView, Error>;
    ...
    constexpr decltype( auto ) operator|( Expected Input, T&& Adaptor ) noexcept
    {
        return std::forward<T>( Adaptor )( Input );
    }

    template<StrViewAdaptable T>
    constexpr Expected& operator|=( Expected& Input, T&& Adaptor ) noexcept
    {
